    ...
    public class EnhancedTriggerBox : MonoBehaviour
    {
        ...
        private void DisplayComponents(List<EnhancedTriggerBoxComponent> components)
        {
            ...
            for (int i = components.Count - 1; i >= 0; i--)
            {
                ...
                {
                    if (components[i]
                        .deleted) // If the X button has been pressed in the top right corner of a component
                    {
        ...
        private void OnTriggerEnter(Collider other)
        {
            if (!disableEntryCheck)
            {
                if (string.IsNullOrEmpty(triggerTags) ||
                    (triggerTags.Split(',').Contains(other.gameObject.tag))) // If it has a valid tag
                {
                    triggered = true;
        ...
        private void OnTriggerExit(Collider other)
        {
            ...
            {
                if (afterTrigger ==
                    AfterTriggerOptions.ExecuteExitResponses) // If we have been told to do something on exit
                {
