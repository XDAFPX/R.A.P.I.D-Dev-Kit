...
namespace DAFP.TOOLS.ECS.UI
{
    public interface IUISystem<TElement> : IUIElement where TElement : IUIElement
    {
        ...
        public TElement GetElement(string name) => GetElements().FirstOrDefault((a => a.Name == name));
        public void HideAll() => GetElements().ForEach((e) => e.Hide());
        public void ShowAll() => GetElements().ForEach((e) => e.Show());
