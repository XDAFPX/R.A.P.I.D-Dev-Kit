...
namespace DAFP.TOOLS.ECS.BuiltIn
{
    public class UniversalSaveSystem : ISaveSystem
    {
        ...
        // {
        //     var _allData =
        //         saveDataService.Load(GetPath(slot).Item1, GetPath(slot).Item2, GetPath(slot).Item3);
        ...
        //     var _metaData =
        //         metaSerializer.Serialize(_allData.GetValueOrDefault("Meta") as GameMetaData);
        //
        ...
        //         {
        //             _presistantEnts.AddOrSet(_entity.ID, serializer.Serialize(_entity));
        //             continue;
        //         }
        //
        //         _worldData.AddOrSet(_entity.ID, serializer.Serialize(_entity));
        //     }
        //
        //
        ...
        //     saveDataService.Save(_allData, GetPath(slot).Item1, GetPath(slot).Item2, GetPath(slot).Item3);
        //
        //     saveMadeEvent.Publish(new(this, serializer, saveDataService));
        ...
        {
            dataService.DeleteSave(GetPath(slot).Item1, GetPath(slot).Item2, GetPath(slot).Item3);
        }

        // public void TryChangeCurrentScene(ISerializationDataService serializationDataService, IMetaSerializer serializer,
        //     int scene, int slot)
        // {
        //     var data = serializationDataService.Load(GetPath(slot).Item1, GetPath(slot).Item2, GetPath(slot).Item3);
        ...
        //     }
        //
        //     serializationDataService.Save(data, GetPath(slot).Item1, GetPath(slot).Item2, GetPath(slot).Item3);
        // }
        ...
        {
            if (slot != -1)
                DeleteSave(saveDataService, -1);
            var _allData = new Dictionary<string, object>();
            _allData = saveDataService.Load(GetPath(slot).Item1, GetPath(slot).Item2, GetPath(slot).Item3);
            ...
            if (_allData.TryGetValue("Worlds", out var _value1))
            {
                ...
                if (_worldsdata.TryGetValue(SceneManager.GetActiveScene().buildIndex.ToString(), out var _value))
                {
                    ...
                    foreach (var _entity in world.Entities)
                    {
                        ...
                        if (_key != default)
                        {
                            serializer.DeSerialize(_worlddata.GetValueOrDefault(_key) as Dictionary<string, object>,
                                _entity);
                            _matchedKeys.Add(_key);
                        }
                        else
                        {
                            serializer.DeSerialize(null, _entity);
                        }
                    }
                    ...
                    foreach (var _unmatchedKey in _unMatched)
                        serializer.DeSerialize(
                            _worlddata.GetValueOrDefault(_unmatchedKey) as Dictionary<string, object>,
                            null);
                }
            }
                    ...
                    foreach (var _entity in world.Entities)
                        if (_entity.GetWorldRepresentation().GetComponent<PresistantEnt>())
                            if (PresistantEnts.TryGetValue(_entity.ID, out var _value2))
                            {
                                ...
                                serializer.DeSerialize(savedata, _entity);
                            }
                }
            }

            saveLoadedEvent.Publish(new(this, serializer, saveDataService));
            // ((IEventBus)Bus).Send(new OnSaveMadeOrLoaded(
            //     saveService.GetFullSavePath(GetPath(slot).Item1, GetPath(slot).Item2, GetPath(slot).Item3), serializer,
            //     saveService,
            //     false));
