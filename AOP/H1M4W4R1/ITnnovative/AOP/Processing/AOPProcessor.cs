    ...
    public static class AOPProcessor
    {
        ...
        public static object OnMethod(object instance, Type type, string methodName, object[] args)
        {
            ...
            if (!arguments.HasErrored)
            {
                try
                {
                    if (method.DeclaringType.IsGenericType && instance != null) // Generic error handle
                    {
                        // Get the method from the concrete type instead
                        var concreteType = instance.GetType();
                        var concreteMethod = concreteType.GetMethod(method.Name);

                        arguments.returnValue = concreteMethod.Invoke(instance, args);
                    }
                    else
                    {
                        arguments.returnValue = method.Invoke(instance, args);
