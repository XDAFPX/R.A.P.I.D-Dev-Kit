...
namespace BDeshi.BTSM.Editor
{
    public class FSMEditorWindow : EditorWindow
    {
        ...
        private bool showDATAToggle = true;
        ...
        private void drawBlackBoard(IRunnableStateMachine fsm)
        {
            showDATAToggle = EditorGUILayout.BeginFoldoutHeaderGroup(showDATAToggle, "CurDATA:");
            if (showDATAToggle)
            {
                using (var scrollViewScope = new EditorGUILayout.ScrollViewScope(DATAScollPos))
                {
                    DATAScollPos = scrollViewScope.scrollPosition;

                    drawBoard(fsm.BlackBoard);
                    GUI.color = oldCol;
                }
            }
        ...
        private void drawBoard(BlackBoard blackBoard)
        {
            if (blackBoard == null) return;
            foreach (string key in blackBoard.GetFullData().Keys)
            {
                GUI.color = blackBoard.GetFullData()[key] != null? Color.green: Color.red;
                EditorGUILayout.LabelField($"{key} : {blackBoard.GetFullData()[key]}");
            }
        }
        ...
        private void drawStateLabel(IState s, int depth = 0)
        {
            ...
            var name = (s.FullStateName).PadLeft(s.FullStateName.Length + depth * 2, '-');
        ...
        void drawTransition(ITransitionBase transition)
        {
